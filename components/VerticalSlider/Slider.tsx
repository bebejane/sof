...
const RNVerticalSlider = React.forwardRef<TSliderRef, TSliderProps>(
	(
		...
		const onGestureStart = (event: GestureStateChangeEvent<PanGestureHandlerEventPayload>) =>
		...
		const onGestureEnd = (event: GestureStateChangeEvent<PanGestureHandlerEventPayload>) =>
		...
		const indicator = useAnimatedStyle(() => {
			const style: ViewStyle = {};
			if (showIndicator) {
				let bottom = ((point.value - min) / (max - min)) * height - renderIndicatorHeight / 2;
